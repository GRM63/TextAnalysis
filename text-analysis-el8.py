...
def main():
    ...
    st.subheader("📂 Φόρτωση Κειμένου")
    
    loading_method = st.radio(
        "Επιλέξτε τρόπο φόρτωσης αρχείου:",
        ["Ανέβασμα από Υπολογιστή", "Φόρτωση από Codespace Directory"],
    ...
    if loading_method == "Ανέβασμα από Υπολογιστή":
        ...
        if text_files:
            ...
            selected_file = st.selectbox(
                "Επιλέξτε αρχείο από το Codespace:",
            ...
            if selected_file:
                try:
                    ...
                    st.success(f"✓ Φορτώθηκε: {selected_file}")
                except Exception as e:
                    st.error(f"Σφάλμα φόρτωσης αρχείου: {str(e)}")
        else:
            st.info("📭 Δεν βρέθηκαν αρχεία .txt στο Codespace directory (/workspaces/TextAnalysis)")
    ...
    if text is not None:
        try:
            ...
            st.caption(f"📄 Ενεργό αρχείο: {filename}")
